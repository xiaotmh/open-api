...
public class RouteServiceImpl implements RouteService {
    ...
    public int add(Route route) throws Exception {
        ...
        if (route.getGatewayApiName().trim().equals("")||route.getGatewayApiName()==null) {
            throw new ClassCastException("输入格式异常");
        }
        if (route.getInsideApiUrl().trim().equals("")||route.getInsideApiUrl()==null) {
            throw new ClassCastException("输入格式异常");
        }
        if (route.getServiceId().trim().equals("")||route.getServiceId()==null) {
            throw new ClassCastException("输入格式异常");
        }
        if (route.getDescription().trim().equals("")||route.getDescription()==null) {
            throw new ClassCastException("输入格式异常");
        }
